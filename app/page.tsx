...
export default function HomePage() {
  ...
  return (
    <main className="academic-bg min-h-screen px-4 py-5 sm:px-6 lg:px-8">
      <section className="mx-auto grid min-h-[calc(100vh-2.5rem)] w-full max-w-[1500px] overflow-hidden rounded-[2rem] bg-white/70 shadow-soft ring-1 ring-slate-200 backdrop-blur sm:rounded-[2.5rem] lg:grid-cols-[0.92fr_1.08fr]">
        <div className="flex flex-col justify-center p-5 sm:p-10 lg:p-14">
          <div className="flex flex-wrap gap-2">
            {["Jogo ao vivo", "Para sala de aula", "EJA", "Matemática"].map((badge) => (
              <span key={badge} className="rounded-full bg-white px-3 py-1.5 text-xs font-black uppercase text-raceBlue shadow-sm ring-1 ring-slate-200">
                {badge}
              </span>
            ))}
          </div>
          <p className="mt-8 text-sm font-black uppercase text-ifGreen">Projeto educacional • Matemática em tempo real</p>
          <h1 className="mt-3 text-[3.2rem] font-black leading-[0.9] text-slate-950 sm:text-7xl xl:text-8xl">
          ...
          <p className="mt-8 text-sm font-bold text-slate-500">Projeto acadêmico de matemática — Corrida das Expressões</p>
        </div>
